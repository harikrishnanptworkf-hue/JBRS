...
function Pending() {
    ...
    // Edit handler for Action column -> navigate to client-create with prefilled data
    const handleEditReport = (row) => {
        const scheduleId = row?.s_id;
        const enquiryId = row?.e_id || row?.enquiry_id;
        if (scheduleId) {
            navigate('/client-create', { state: { editId: scheduleId, editType: 'schedule', from: 'invoice' } });
            return;
        }
        if (enquiryId) {
            navigate('/client-create', { state: { editId: enquiryId, editType: 'enquiry', from: 'invoice' } });
            return;
        }
        toast.warn('No associated schedule or enquiry to edit.');
    };
            ...
            cell: (cellProps) => (
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '100%' }}>
                    <button
                        ...
                        id={`edittooltip-${cellProps.row.original.id || cellProps.row.original.invoice_id || cellProps.row.original.s_id || cellProps.row.index}`}
                        style={{ color: '#1a8cff', background: '#e6f2ff' }}
                        onClick={() => handleEditReport(cellProps.row.original)}
                    >
    ...
    ], [sortBy, sortOrder, handleSortChange, handleEditReport]);
