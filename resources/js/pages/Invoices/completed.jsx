...
function Completed() {
    ...
    const columns = useMemo(() => [
        ...
        {
            header: (
                <span style={{ display: 'inline-flex', alignItems: 'center', cursor: 'pointer' }} onClick={() => handleSortChange('s_invoice_number')}>
                    Invoice No
                    {sortBy === 's_invoice_number' && (
                        <span style={{ marginLeft: 6, fontSize: 16, color: '#ffffffff' }}>{sortOrder === 'asc' ? '▲' : '▼'}</span>
                    )}
                </span>
            ),
            accessorKey: 's_invoice_number',
            enableSorting: true,
            cell: (cellProps) => <span>{cellProps.row.original.s_invoice_number || ''}</span>
        },
        {
            header: (
                <span style={{ display: 'inline-flex', alignItems: 'center', cursor: 'pointer' }} onClick={() => handleSortChange('s_amount')}>
                    Amount
                    {sortBy === 's_amount' && (
                        <span style={{ marginLeft: 6, fontSize: 16, color: '#ffffffff' }}>{sortOrder === 'asc' ? '▲' : '▼'}</span>
                    )}
            ...
            accessorKey: 's_amount',
            enableSorting: true,
            cell: (cellProps) => <span>{rupee(cellProps.row.original.s_amount)}</span>
        },
        {
        ...
        // Removed Comment (Revoke comment) column as per requirement
        // Actions: Download PDF icon
        {
            header: 'Actions',
            id: 'actions',
            enableSorting: false,
            cell: (cellProps) => {
                ...
                const scheduleId = row.s_id || row.id;
                const invoiceNumber = row.s_invoice_number;
                const canDownload = !!invoiceNumber && !!scheduleId;
                const onDownload = async () => {
                    try {
                        const res = await api.get(`/invoice/download/${scheduleId}`, { responseType: 'blob' });
                        ...
                        try { toast.error('Failed to download invoice PDF'); } catch (e) {}
                    }
                };
                return (
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                        <button
                            ...
                            style={{ color: canDownload ? '#1a8cff' : '#999', background: canDownload ? '#e6f2ff' : '#f0f0f0' }}
                        >
        ...
        <React.Fragment>
            ...
            `}</style>
                ...
                <div style={{ padding: '32px 32px 32px 32px', width: '100%', background: '#fff' }}>
                                        {/* Ensure standard font for table rows */}
                                        <style>{`
                                                .reminder-table-shadow table, .reminder-table-shadow th, .reminder-table-shadow td {
