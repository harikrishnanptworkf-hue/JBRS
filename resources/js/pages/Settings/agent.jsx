...
const AgentCore = () => {
  ...
  const columns = React.useMemo(() => [
    ...
    },
    {
      ...
      cell: row => {
        ...
        return row.row.original.password || '';
      },
    },
    {
      id: 'action',
  ...
  ], [editRowId, sortBy, sortDirection]);
  ...
  const handleEditSave = async (id) => {
    ...
    if (!nameToSave.trim() || !usernameToSave.trim() || !passwordToSave.trim()) {
      showSnackbar('Please enter all fields', 'error');
      return;
    }
    setIsSavingEdit(true);
    try {
      await api.put(`/agents/${id}`, { name: nameToSave, username: usernameToSave, password: passwordToSave });
      showSnackbar('Agent updated', 'success');
      setEditRowId(null);
      setEditName('');
      setEditUsername('');
      setEditPassword('');
  ...
  return (
      ...
      {/* Header Bar: Title only */}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 18, justifyContent: 'flex-start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            <div className="examcode-title-text">Agent</div>
      ...
      <div style={{ maxHeight: 800, opacity: 1, overflow: 'hidden', transition: 'max-height 0.5s cubic-bezier(.4,0,.2,1), opacity 0.4s' }}>
        {/* Form Row: Name/Username/Password/Add */}
        <div className="examcode-form-row">
          ...
          <div>
            <Button className="examcode-create-btn" onClick={handleAddAgent} disabled={loading}>Create</Button>
          </div>
        </div>
